...
A vehicle is eligible only if a licensed member of its owner household is among the people it would carry. In the previous change the check ran after assignment and dropped whole vehicles, stranding 10,849 people who by then had nowhere else to go. Choosing eligibility up front means those people are simply seated elsewhere. This also removes an entire class of correctness bug: the previous implementation counted drivers per travel group, which let a split family's single licensed member make both of its vehicles look drivable.

**The owner household loses its claim to a seat, but keeps its car.**
Alternative considered: keep the owner household's first claim on its own vehicle and optimise only the remaining seats — rejected, because that is precisely the constraint that pins the car count. But the opposite extreme, letting any licensed passenger drive any car, was measured and rejected too: it produced 28,222 departing cars of which only 1,861 were driven by their owner, implying 26,361 key handovers between households in an evacuation. A car is now driven by its owner or not at all. This costs vehicles — a car whose owner is not travelling cannot be used, however well placed — and that cost is the price of a plan that does not depend on strangers exchanging keys under pressure.
...
- [Trade-off] Vehicle activation order makes the result sensitive to tie-breaking → accepted, and made deterministic by ordering ties on vehicle id, so runs are reproducible.
