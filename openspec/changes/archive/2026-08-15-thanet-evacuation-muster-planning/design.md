...
- Assign every synthetic household a real, open-data-sourced home location, identified by UPRN and coordinates.
...
**Dwelling source: the ONS National Statistics UPRN Lookup alone.**
NSUL carries, per UPRN, both British National Grid coordinates and the Output Area allocation, which is everything the pipeline needs from a dwelling dataset. Alternatives considered and rejected: (a) OS Open UPRN for coordinates plus NSUL for allocation, as the first draft specified — rejected once NSUL's schema was inspected and found to carry `GRIDGB1E`/`GRIDGB1N`, making OS Open UPRN a 618 MB download contributing nothing; (b) an OS Open UPRN to Overture Buildings to OSM fallback chain, from the original draft — rejected because OS Open UPRN derives from AddressBase and covers every addressable object in GB, so the fallback branch never executes, and Overture Buildings carries no addresses either. NSUL is also published split by region, and a single region member can be extracted from the archive by range request: 71 MB for the South East rather than 514 MB for the whole of GB.

**No human-readable address is produced.**
The first draft required an address string, first sourced (impossibly) from OS Open UPRN, then synthesized from OS Open USRN street names plus a generated building number. Dropped on the explicit instruction that a readable address is not wanted. Homes are identified by UPRN and coordinates, which is what the downstream congestion model consumes and what physically locates a person for collection. This also drops the OS Open USRN dependency (a 297 MB GeoPackage, its only consumer being the address string) and removes the awkwardness of attaching a plausible-looking address to a fictional household.
...
**Vehicles snap to the nearest road unconditionally, with no distance cap.**
An earlier draft dropped any vehicle whose nearest drivable road exceeded 20 m. Measured against real data that removed 19,038 of 70,098 vehicles — 27% of the fleet, roughly 95,000 seats — before the ride-share question was even asked. Investigating the excluded vehicles showed a median nearest road of 23.0 m, with 85% within 30 m: the dwellings are not road-less, OpenStreetMap simply has not mapped their access roads, service roads and driveways. Since a UPRN sits at a building's access point rather than at the kerb, front gardens and set-back terraces routinely exceed 20 m. Raising the cap was considered and rejected as arbitrary — any threshold encodes a guess about map completeness. Every vehicle now parks at its nearest road point whatever the distance, and the snap distance is reported as a data-quality signal rather than used as a filter.
...
- [Risk] Some synthetic households will be placed at non-residential UPRNs (shops, offices, parent-shell records for blocks of flats), since NSUL carries no classification → Mitigation: report the UPRN-to-household ratio per Output Area; the effect is diluted by surplus UPRNs and does not affect the point's street or Output Area, which is what the downstream model consumes.
- [Risk] NSUL is keyed to an AddressBase epoch and refreshed roughly six-weekly, so the UPRN set shifts between editions and a rerun months later may place households slightly differently → Mitigation: record the NSUL epoch used in the ingestion summary, so a population can be tied to the edition it was built from.
- [Risk] Snap distances are inflated by OpenStreetMap's incomplete coverage of residential access roads, service roads and driveways, so a vehicle's recorded parking point can sit further from its home than it would in reality → Mitigation: no vehicle is dropped for this (see the parking decision below); the snap-distance distribution and the count beyond a review threshold are reported, so the scale of the map gap stays visible.
